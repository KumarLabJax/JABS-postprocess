...
def mock_empty_bout_table():
    ...
    """
    mock = MagicMock()
    mock._data = pd.DataFrame(columns=['video_name', 'animal_idx', 'start', 'duration', 'is_behavior'])
    return mock
...
class TestEvaluateGroundTruth:
    ...
    def test_evaluate_ground_truth_basic(
        ...
        # Assertions
        mock_from_jabs_annotation_folder.assert_called_once_with('test_gt_folder', 'test_behavior')
    ...
    def test_evaluate_ground_truth_custom_params(
        ...
        assert call_args[4] == True  # filter_ground_truth


@pytest.mark.parametrize(
    "stitch_scan,filter_scan,threshold_scan,expected_calls",
    [
        ([5, 10], [5, 10], [0.5], 4),  # 2 stitch × 2 filter × 1 threshold = 4 combinations
        ([5], [5, 10, 15], [0.5], 3),  # 1 stitch × 3 filter × 1 threshold = 3 combinations
        ([5, 10], [5], [0.5, 0.6], 4)   # 2 stitch × 1 filter × 2 threshold = 4 combinations
    ]
)
def test_generate_iou_scan_parameter_combinations(
    stitch_scan, filter_scan, threshold_scan, expected_calls, 
    sample_annotations, sample_bouts_objects
):
    """
    ...
    with patch('jabs_postprocess.utils.project_utils.Bouts') as mock_bouts_class:
        ...
        with patch.object(Bouts, 'calculate_iou_metrics', return_value={
            'tp': 1, 'fn': 1, 'fp': 1, 'pr': 0.5, 're': 0.5, 'f1': 0.5
        }):
            # Run the function
...
def test_generate_iou_scan_no_valid_pairs():
    ...
    expected_columns = ['stitch', 'filter', 'threshold', 'tp', 'fn', 'fp', 'pr', 're', 'f1']
...
def test_generate_iou_scan_filter_ground_truth():
    ...
    with patch('jabs_postprocess.utils.project_utils.Bouts.filter_by_settings') as filter_spy:
        ...
        assert filter_spy.call_count == 1, "With filter_ground_truth=False, only predictions should be filtered"
        ...
        assert filter_spy.call_count == 2, "With filter_ground_truth=True, both GT and predictions should be filtered"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {'tp': 5, 'fn': 0, 'fp': 0}, 
            {'pr': 1.0, 're': 1.0, 'f1': 1.0}
        ),
        # FP only
        (
            {'tp': 0, 'fn': 0, 'fp': 5}, 
            {'pr': 0.0, 're': np.nan, 'f1': np.nan}
        ),
        # FN only
        (
            {'tp': 0, 'fn': 5, 'fp': 0}, 
            {'pr': np.nan, 're': 0.0, 'f1': np.nan}
        ),
        # Mixed case
        (
            {'tp': 5, 'fn': 2, 'fp': 3}, 
            {'pr': 0.625, 're': 0.714, 'f1': 0.667}
        )
    ]
)
