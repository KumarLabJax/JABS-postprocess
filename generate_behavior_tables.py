...
def generate_behavior_tables(args, behavior_args: str, linking_dicts: dict={}, experiment_dist_dicts: dict={}):
	...
	for cur_experiment in exp_folders:
		...
		if cur_experiment in linking_dicts.keys():
			experiment_data, _ = read_experiment_folder(cur_experiment, behavior_args.behavior, interpolate_size=behavior_args.interpolate_size, stitch_bouts=behavior_args.stitch_gap, filter_bouts=behavior_args.min_bout_length, linking_dict=linking_dicts[cur_experiment], activity_dict=distance_dict)
		else:
			experiment_data, linking_dict = read_experiment_folder(cur_experiment, behavior_args.behavior, interpolate_size=behavior_args.interpolate_size, stitch_bouts=behavior_args.stitch_gap, filter_bouts=behavior_args.min_bout_length, activity_dict=distance_dict)
...
def main(argv):
	...
	for behavior_set in by_sets(argv, '--behavior'):
		...
		behavior_args.append(new_args)

	available_behaviors = get_behaviors_in_folder(args.project_folder)
	...
	for cur_behavior_args in behavior_args:
		cur_table, linking_dicts = generate_behavior_tables(args, cur_behavior_args, linking_dicts, activity_dicts)
