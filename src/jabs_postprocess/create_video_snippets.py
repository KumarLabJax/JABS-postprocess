...
def create_video_snippet(
	...
	if os.path.exists(output_video) and not overwrite:
		raise FileExistsError(f'{output_video} exists. Set overwrite=True if you wish to overwrite it.')
	...
	write_video_clip(input_video, output_video, frame_range, behavior_data, pose_for_video)
