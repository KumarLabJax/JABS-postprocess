...
def _compute_framewise_confusion(gt_df, pred_df):
    ...
    def expand_intervals_to_frames(df):
        ...
        expanded['frame'] = expanded.apply(
            lambda row: range(row['start'], row['start'] + row['duration']),
            axis = 1
        )
    ...
    confusion_counts = (
        ...
        .apply(lambda x: pd.Series({
            'TP': ((x['is_behavior_gt'] == 1) & (x['is_behavior_pred'] == 1)).sum(),
...
def _find_outliers(melted_df: pd.DataFrame):
    ...
    outliers = pd.concat(outliers) if outliers else pd.DataFrame(columns=melted_df.columns)
...
def generate_framewise_performance_plot(
    ...
    confusion_counts['frame_total'] = (
        ...
        confusion_counts['FP'] + confusion_counts['FN']
    )
    
    # Compute per-video metrics
    confusion_counts['precision'] = confusion_counts['TP'] / (confusion_counts['TP'] + confusion_counts['FP'])
    confusion_counts['recall'] = confusion_counts['TP'] / (confusion_counts['TP'] + confusion_counts['FN'])
    confusion_counts['f1_score'] = 2 * (confusion_counts['precision'] * confusion_counts['recall']) / (confusion_counts['precision'] + confusion_counts['recall'])
    confusion_counts['accuracy'] = (confusion_counts['TP'] + confusion_counts['TN']) / confusion_counts['frame_total']  
    ...
    overall_metrics['f1_score'] = 2 * (overall_metrics['precision'] * overall_metrics['recall']) / (overall_metrics['precision'] + overall_metrics['recall'])   
