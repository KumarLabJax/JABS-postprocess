...
from jsbs_utils.bout_utils import filter_bouts_by_bouts
...
def filter_behavior_tables(input_table_file: os.path, filter_table_file: os.path, out_prefix: str, mode: str, gap_tolerance: int, overlap_tolerance: int, discard_bouts: bool=False):
	...
	output_args = SimpleNamespace(out_prefix = args.out_prefix, project_folder = header_in['Project Folder'][0], interpolate_size = header_in['Interpolate Size'][0], stitch_gap = header_in['Stitch Gap'][0], min_bout_length = header_in['Min Bout Length'][0], out_bin_size = header_in['Out Bin Size'][0])
